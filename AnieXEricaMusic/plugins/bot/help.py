...
from AnieXEricaMusic.utils.inline.help import help_back_markup, private_help_panel, AMBOThelp_back_markup, AMBOTprivate_help_panel
...
@app.on_message(filters.command(["AMBOThelp"]) & filters.private & ~BANNED_USERS)
@app.on_callback_query(filters.regex("AMBOTsettings_back_helper") & ~BANNED_USERS)
async def AMBOThelper_private(
    client: app, update: Union[types.Message, types.CallbackQuery]
):
    is_callback = isinstance(update, types.CallbackQuery)
    if is_callback:
        ...
        language = await get_lang(chat_id)
        _ = get_string(language)
        keyboard = AMBOThelp_pannel(_, True)
        await update.edit_message_text(
            _["help_1"].format(SUPPORT_CHAT), reply_markup=keyboard
        )
    else:
...
@app.on_message(filters.command(["AMBOThelp"]) & filters.group & ~BANNED_USERS)
@LanguageStart
async def help_com_group(client, message: Message, _):
    keyboard = AMBOTprivate_help_panel(_)
    await message.reply_text(_["help_2"], reply_markup=InlineKeyboardMarkup(keyboard))


@app.on_callback_query(filters.regex("AMBOThelp_callback") & ~BANNED_USERS)
@languageCB
async def helper_cb(client, CallbackQuery, _):
    callback_data = CallbackQuery.data.strip()
    cb = callback_data.split(None, 1)[1]
    keyboard = AMBOThelp_back_markup(_)
    if cb == "hb1":
        await CallbackQuery.edit_message_text(helpers.HELP_1, reply_markup=keyboard)
    elif cb == "hb2":
        await CallbackQuery.edit_message_text(helpers.HELP_2, reply_markup=keyboard)
